...
def load_dataset():
    ...
    X_train, X_test = X_train[:-25], X_train[-25:]
    y_train, y_test = y_train[:-25], y_train[-25:]
...
def run_network():
    ...
    while True:
        ...
        # Load the dataset
        print("Loading data...")
        X_train, y_train, X_val, y_val, X_test, y_test,modelXtest,modelYtest,X_train_forTesting,testSet = load_dataset()
        ...
        test_acc = 0
        test_batches = 0
        for batch in iterate_minibatches(modelXtest, modelYtest,25, shuffle=False):
            inputs, targets = batch
            err, acc = val_fn(inputs, targets)
            test_err += err
            test_acc += acc
        ...
        if(accuracy > 96.5):
            ...
            for sample in p:
                for item in sample:
                        ...
                        fp.append(MaxClass)
            fp = np.asarray(fp)
            f = open('q12predictions_nn.txt', 'w')
            for pred in fp:
                s = str(pred)
                f.write(s)
                f.write("\n")
            f.close()
            print("-----------------------------------------------------------")
            print("Predictions available for viewing in q12predictions_nn.txt")
            print("------------------------------------------------------------")
            plt.show()
            return
