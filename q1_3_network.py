...
def load_dataset():
    ...
    X_train = pca.transformed_trainData3_norm.values
    y_train = dp.TrainData3_labels.values
    ...
    testSet = np.array(pca.transformed_testData3_norm.values, dtype=np.float32)
    ...
    X_train, X_test = X_train[:-10], X_train[-10:]
    y_train, y_test = y_train[:-10], y_train[-10:]
    ...
    return X_train , y_train, X_val, y_val, X_test, y_test,modelXtest,modelYtest,testSet
    '''
    #this is raw data
...
def build_mlp(input_var=None):
    ...
    l_in = lasagne.layers.InputLayer(shape=(None,14),
                                     input_var=input_var)
    ...
    l_hid1 = lasagne.layers.DenseLayer(
            l_in_drop, num_units=14,
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())

    # We'll now add dropout of 30%:
    l_hid1_drop = lasagne.layers.DropoutLayer(l_hid1, p=0.3)

    # Finally, we'll add the fully-connected output layer, of 11 softmax units:
    l_out = lasagne.layers.DenseLayer(
            l_hid1_drop, num_units=11,
...
def run_network():
    ...
    while True:
        ...
        # Load the dataset
        print("Loading data...")
        X_train, y_train, X_val, y_val, X_test, y_test,modelXtest,modelYtest,testSet = load_dataset()
        ...
        # After training, we compute and print the model error:
        test_err = 0
        test_acc = 0
        test_batches = 0
        for batch in iterate_minibatches(X_test, y_test, 5, shuffle=False):
        ...
        if(accuracy > 98):
            ...
            for sample in p:
                for item in sample:
                        ...
                        fp.append(MaxClass)
            fp = np.asarray(fp)
            f = open('q13predictions_nn.txt', 'w')
            for pred in fp:
                s = str(pred)
                f.write(s)
                f.write("\n")
            f.close()
            print("-----------------------------------------------------------")
            print("Predictions available for viewing in q13predictions_nn.txt")
            print("------------------------------------------------------------")
            plt.show()
            return
